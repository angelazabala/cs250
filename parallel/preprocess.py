...
def get_eng_words(content_array, mail, rank):
    filtered = open(str(rank)+"data2"+mail[4:], "w")
...
def clean_email(mail, rank):
    ...
    get_eng_words(words, mail, rank)

def preprocess(start, end, rank):
    if not os.path.exists(str(rank)+"data2/"):
        os.makedirs(str(rank)+"data2/")

    for i in range(start, end):
        print("Preprocessing data/inmail."+str(i))
        clean_email("data/inmail."+str(i), rank)